...
common_reason %>% 
  pivot_longer(
      cols = (1:6),
    names_to = "Sanitary_infrastructure",
    values_to = "response")

  # ) %>% 
  # dplyr::filter(response=="available") %>% 
  # count(reason,Sanitary_infrastructure) %>% 
  # pivot_wider(
  #   values_from = n,
  #   names_from =reason
  # ) %>% 
  #  mutate(
  #   percentage=     Cost/(Cost+Distance+`Poor Maintenance/Unhygienic`)*100, 
  # 
  #    label = paste0(Cost, " (", round(percentage, 1), "%)")
  #   
  # )
