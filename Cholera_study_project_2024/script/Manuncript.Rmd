...
ci_table_san<- table_san_facility|>
  tbl_summary()|>
  # add_p()|>
  add_ci()

ci_table_san<- ci_table_san|>
  as_flex_table()|>
  autofit()

save_as_docx(ci_table_san, 
             path = "output/ci_table_san.docx")
...
summarytools::freq(practice_df$health_risk_associated_with_drinking_untreated_water)
...
## Usage of Sanitary Infractures in the Community and Household Level

```{r}
cholera_data %>% 
  select(
    shared_usage_of_toilets_latrines, 
    san_facility_unavailable
  ) %>% 
  summarytools::freq()
...
# Regession Analysis on the sanitary infractures and cholera vaccination.
...
summarytools::freq(san_model)

unique(san_model$sanitary_and_infrastructure_category)
           ...
           save_as_docx(Crudes_OR, 
                        path = "output/Crudes_OR.docx")
  ...
  save_as_docx(Adjusted_OR, 
               path = "output/Adjusted_OR.docx")


  View(sanitary_by_person)
...
fisher_test1<- fisher.test(sanitary_by_person$sanitary_and_infrastructure_category, sanitary_by_person$occupation,                            simulate.p.value = T)
...
```{r}
      ...
      OCV_awareness<- cholera_data[grep( "information_about_ocv" , colnames(cholera_data))] 



OCV_awareness<- OCV_awareness %>% 
  pivot_longer(
    cols = (2:13),
    names_to = "variable",
    values_to = "response"
...
OCV_awareness$variable<- gsub("information_about_ocv_",
                              " ",
                              OCV_awareness$variable)

OCV_awareness$variable<- gsub("_",
                              " ",
                              OCV_awareness$variable)


OCV_awareness<- OCV_awareness %>% 
  ggplot(aes(x = fct_reorder(variable, percent), y = percent)) +
  geom_col(fill = "skyblue") +
  coord_flip() +
  theme_minimal()+
  labs(
    x = "Source of OCV Information",
    y = "Percentage (%)",
    title = "Main source of OCV Information among Study Participants"
  )+
  geom_text(aes(label = paste0(round(percent, 1), "%")), 
...
ggsave(here("output/OCV_awareness.png"),
       OCV_awareness, 
       width = 10, 
       height = 10, 
       bg = "white", 
...
=======
WATER TREATMENT
>>>>>>> 6cd55e75b35b95405dd479e481a50d87c6416afa
